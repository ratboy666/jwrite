...
static void jwPutraw(jwc_t *jwc, const char *str);
...
static int _jwObj(jwc_t *jwc, const char *key);
 ...
 */
void
jwOpen(jwc_t *jwc, char *buffer, size_t buflen, 
 ...
 */
void
jwObj_raw(jwc_t *jwc, const char *key, char *rawtext)
{
 ...
 */
void
jwObj_string(jwc_t *jwc, const char *key, char *value)
{
	if (_jwObj(jwc, key) == JWRITE_OK)
		jwPutstr(jwc, value);
}
...
void
jwObj_double(jwc_t *jwc, const char *key, double value)
{
    int n;
 ...
 */
void
jwObj_object(jwc_t *jwc, const char *key)
{
	if (_jwObj(jwc, key ) == JWRITE_OK) {
		jwPutch(jwc, '{');
 ...
 */
void
jwObj_array(jwc_t *jwc, const char *key)
{
	if (_jwObj(jwc, key) == JWRITE_OK) {
		jwPutch(jwc, '[');
		jwPush(jwc, JW_ARRAY);
 ...
 */
void
jwArr_raw(jwc_t *jwc, char *rawtext)
{
 ...
 */
void
jwArr_string(jwc_t *jwc, char *value)
{
	if (_jwArr(jwc) == JWRITE_OK)
		jwPutstr(jwc, value);
}
...
void
jwArr_double(jwc_t *jwc, double value)
{
	int n;
	n = snprintf(jwc->tmpbuf, sizeof (jwc->tmpbuf), "%g", value);
	if ((n >= sizeof (jwc->tmpbuf)) || (n < 0))
 ...
 */
static void
jwPush(jwc_t *jwc, enum jwNodeType nodeType)
{
 ...
 */
static void
jwPutraw(jwc_t *jwc, const char *str)
{
	while (*str != '\0')
		jwPutch(jwc, *str++);
}
 ...
 */
static int
_jwObj(jwc_t *jwc, const char *key)
{
	if (jwc->error == JWRITE_OK) {
		++jwc->callNo;
